...
(define-metafunction L
  mc_s : fn p E -> S
  [(mc_s fn p E) S (where (Pr S K) (mc fn p E))])

(define-metafunction L
  mc_k : fn p E -> K
  [(mc_k fn p E) K (where (Pr S K) (mc fn p E))])
...
(define-metafunction L
  ...
  [(mc fn (seq p q) E)
   (mc fn p E)
   (where #false (∈ 0 (mc_k fn p E)))]

  [(mc fn (seq p q) E)
   (Pr (∪ (mc_s fn p E) (mc_s fn_2 q E))
       (∪ (set-subtract (mc_k fn p E) 0)
          (mc_s fn_2 q E)))
  ...
  [(mc fn (par p q) E)
   (Pr (∪ (mc_s fn p E) (mc_s fn q E))
       (max (mc_k fn p E) (mc_k fn q E)))]
  ...
  [(mc Must (p \\ s) E)
   (mc Must p (extend s tt E))
   (where #true (∈ s (fn_s Must p (extend s ⊥ E))))]
  ...
  [(mc Must (p \\ s) E)
   (remove-s (mc Must p (extend s ff E)) s)
   (where #false (∈ s (fn_s Can+ p (extend s ⊥ E))))]
